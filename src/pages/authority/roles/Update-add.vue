<template>
  <el-dialog :visible.sync="isShow" :before-close='dialogClosed' @open="dialogOpened" width="1050px" :close-on-click-modal='false'>
    <span slot="title">{{title}}</span>
    <el-form :model="form" v-loading="formDataloading" ref="Form" :rules="rules" status-icon inline>
      <el-row>
        <el-form-item label="角色名称：" :label-width="formLabelWidth" class="form-item" prop="name">
        ...
        <el-form-item label="角色描述：" :label-width="formLabelWidth" style="height: 48px" class="form-item" prop="descript">
          <el-input type="textarea" v-model="form.descript" size="medium"></el-input>
        </el-form-item>
      </el-row>
      <el-row type="flex" justify="center">
        <el-form-item prop="menus">
          <el-transfer v-loading='transLoading' v-model="form.menus" filterable :titles="['所有菜单', '所选菜单']" :button-texts="['到左边', '到右边']" :data="menusList" />
        </el-form-item>
      </el-row>
...
<script>
import { queryMenuBySys, queryMenuData } from '@/api/menuManage';
import { querySysData  } from "@/api/sysManage";
import {addRoleData, updateRoleData} from "@/api/rolesManage";
